...
PORT = sys.argv[2]      #Taking second argument from terminal 
...
def random_list():
    ...
    for i in range(NUM_MSGs):
        data[i] = "{process}:{sleep_time}".format(process = random.choice(process_list), sleep_time = random.randint(1, RST))
    return data 
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        for msg in msg_list:
            try:
                ...
                time.sleep(random.randrange(1, RST))                    #Sleeping a random amount of time between 1 and RST value
